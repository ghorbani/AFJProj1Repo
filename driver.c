...
void measure_performance () {
  ...
  A = oA = (double *) malloc(SQR(test_sizes[NUM_TESTS - 1]) * sizeof(double));
  B = oB = (double *) malloc(SQR(test_sizes[NUM_TESTS - 1]) * sizeof(double));
  C = oC = (double *) malloc(SQR(test_sizes[NUM_TESTS - 1]) * sizeof(double));

  if (!A || !B || !C) {
    printf("measure_performance: malloc() failed\n");
    exit(-1);
  }
